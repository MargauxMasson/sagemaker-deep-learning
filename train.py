...
class SaveCheckpoints(keras.callbacks.Callback):
    ...
    def __init__(self, base_name_checkpoint, random_id_training):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        s3.Bucket(BUCKET).upload_file(model_name, checkpoint_s3_uri)
...
if __name__ == '__main__':
    ...
    config = wandb.config
    # update(allow_val_change=True)   # https://github.com/shagunsodhani/ml-logger/issues/74

    epochs = args.epochs
    lr = args.learning_rate
    batch_size = args.batch_size
    augmentation_data = args.augment
    aug_flip_left_right = args.flip_left_right
    aug_flip_up_down = args.flip_up_down
    aug_rot90 = args.rot90
    ...
    print(f"training_dir {os.path.join(training_dir, 'labels_test_v1.csv')}")
    print(f"val_dir {os.path.join(validation_dir, 'val_labels.csv')}")

    print("Dataloader initialization...")
    gen = DataLoader(label_file_path_train=os.path.join(training_dir, 'labels_test_v1.csv'),
                     label_file_path_val=os.path.join(validation_dir, 'val_labels.csv'),
                     bucket_name='margaux-bucket',
                     data_extension_type='.tif',
                     bands=bands,
    ...
    save_checkpoint_wandb = SaveCheckpoints(base_name_checkpoint, random_id_training)
